...
class KubectlExecutor(CommandExecutor):
    ...
    def __init__(self, default_timeout: int = 30, namespace: Optional[str] = None):
    ...
    async def execute(
        ...
        **kwargs
        ...
        Args:
            command: kubectl 子命令（如 "get pods"）
            user_id: 用户ID
            db: 数据库会话
            timeout: 超时时间
        ...
        if command.strip().startswith('kubectl'):
            ...
            if ns and "-n" not in command and "--namespace" not in command and "-A" not in command:
                full_command = command.replace('kubectl', f'kubectl -n {ns}', 1)
        else:
            full_command = f"kubectl {command}"
            if ns and "-n" not in command and "--namespace" not in command and "-A" not in command:
                full_command = f"kubectl -n {ns} {command}"
